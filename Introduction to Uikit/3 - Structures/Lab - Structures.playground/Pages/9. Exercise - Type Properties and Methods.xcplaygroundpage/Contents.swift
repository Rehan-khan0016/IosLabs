...
struct User {
    ...
    var email: String
    var age: Int
    static var currentUser = User(userName: "jagpreet", email: "jagpreet@gmail.com", age: 120
    static func logIn(user: User) {
        currentUser = user
        print("\(user.userName) has been logged.")
    }
}
print("current user's username is: \(User.currentUser.userName) whose email is: \(User.currentUser.email) and age is: \(User.currentUser.age)")
 ...
 */
let newUser = User(userName: "new", email: "new@gmail.com", age: 20)
User.logIn(user: newUser)
print("Current logged in user is: \(User.currentUser.userName)")
